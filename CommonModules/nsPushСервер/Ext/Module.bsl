...
Функция ОтправитьPUSH_Android(ИдентификаторПодписчика, Title, Data, Text) Экспорт
	Попытка
        ...
        ПараметрыСообщения.Вставить("to", ИдентификаторПодписчика.ИдентификаторУстройства);
		...
		Если НЕ Ответ.КодСостояния = 200 Тогда
			Возврат nsОбработкаОшибок.ЗаписатьОшибку("Ошибка отправки PUSH сообщения. КодСостояния = " + Ответ.КодСостояния + Символы.ПС + Ответ.ПолучитьТелоКакСтроку(), "PushСервер\ОтправитьPUSH_Android");
		КонецЕсли; 
